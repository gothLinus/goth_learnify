                ...
                <form class="space-y-6" method="post" action="/card/edit/{{ $card->id }}" enctype="multipart/form-data">
                    ...
                    <div>
                        ...
                        <textarea name="title" required
                                  class="w-full text-gray-800 dark:text-white dark:bg-gray-700 text-sm sm:text-base border border-gray-300 dark:border-gray-600 px-4 py-3 rounded-md outline-none focus:ring-2 focus:ring-blue-500 transition-all duration-200 h-12"
                    ...
                    <div class="mt-4 flex flex-wrap gap-2">
                        ...
                        <template x-for="(file, index) in newFiles" :key="index">
                            <div class="relative">
                                <div class="file-preview-item p-2 border rounded-lg dark:border-gray-600">
                                    <template x-if="file.isImage">
                                        <img :src="file.preview" alt="nothing"
                                             class="w-20 h-20 object-cover rounded-md">
                                    </template>
                                    <template x-if="!file.isImage">
                                        <div class="p-2 bg-gray-100 dark:bg-gray-700 rounded-md">
        ...
        function fileManager( existingFiles ) {
            return {
                existingFiles: existingFiles.map(file => ({
                    id: file.id,
                    name: file.name,
                    preview: file.is_image ? src = file.url : '', //what needs to be here?
