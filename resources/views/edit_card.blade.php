<x-layout>
    <div class="min-h-screen flex flex-col items-center justify-center py-6 px-4 sm:px-6 md:px-8"
         x-data="filePreview({{ $card->files->map(fn($file) => ['name' => $file->name, 'type' => mime_content_type(storage_path('app/public/' . $file->path)), 'preview' => asset('storage/' . $file->path), 'file' => $file])->toJson() }})">
        <div class="w-full max-w-sm sm:max-w-md lg:max-w-lg">
            <div class="p-6 sm:p-8 lg:p-10 rounded-2xl bg-white shadow-md">
                <h2 class="text-gray-800 text-center text-2xl font-bold sm:text-3xl lg:text-4xl">Edit Card</h2>
                <form class="mt-6 sm:mt-8 space-y-4" method="post" action="/card/edit/{{$card->id}}"
                    ...
                    <div>
                        <label class="text-gray-800 text-sm sm:text-base mb-2 block" for="multiple_files">Upload
                            files</label>
                        ...
                        <template x-for="(file, index) in files" :key="index">
                            <div class="file-preview-item mt-2 relative">
                                <template x-if="file.type.startsWith('image/')">
                                    <img :src="file.preview" :alt="file.name" class="w-20 h-20 object-cover rounded-md">
                                </template>
                                <template x-if="!file.type.startsWith('image/')">
                                    <p x-text="file.name" class="text-gray-800"></p>
                                </template>
                                <button @click="removeFile(index, file.id)" type="button"
                    ...
                    <div>
                        <label for="time" class="text-gray-800 text-sm sm:text-base mb-2 block">Select time:</label>
        ...
        function filePreview( initialFiles = [] ) {
            return {
                ...
                removeFile( index, fileId ) {
                    ...
                    const deletedFilesInput = this.$el.querySelector('input[name="deleted_files"]');
